...
public class KColorsAlgorithm<T> implements GraphColoringAlgorithm<T> {
    ...
    @Override
    public void execute(Graph<GraphColoringNode<T>> graph, int startingColorValue) {
        for(var node: graph.getNodes()) {
            ...
            if((tentativeColor - startingColorValue) <= numberOfColors) {
                possibleColorFound = true;
            }

            if(possibleColorFound) node.setColor(tentativeColor);
            else {
                // report error
            }
        }
